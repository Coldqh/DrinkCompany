...
export function parseGameState(serialized: string): GameState {
  try {
    ...
    if (!Number.isInteger(parsed.schemaVersion) || (parsed.schemaVersion ?? 0) < 1 || (parsed.schemaVersion ?? 0) > 25) throw new Error('Файл не является сохранением Drink Company');
