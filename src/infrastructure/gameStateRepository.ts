...
export function parseGameState(serialized: string): GameState {
  try {
    ...
    if (parsed.schemaVersion !== 1 && parsed.schemaVersion !== 2 && parsed.schemaVersion !== 3 && parsed.schemaVersion !== 4 && parsed.schemaVersion !== 5 && parsed.schemaVersion !== 6 && parsed.schemaVersion !== 7 && parsed.schemaVersion !== 8 && parsed.schemaVersion !== 9 && parsed.schemaVersion !== 10 && parsed.schemaVersion !== 11 && parsed.schemaVersion !== 12 && parsed.schemaVersion !== 13 && parsed.schemaVersion !== 14 && parsed.schemaVersion !== 15 && parsed.schemaVersion !== 16) throw new Error('Файл не является сохранением Drink Company');
