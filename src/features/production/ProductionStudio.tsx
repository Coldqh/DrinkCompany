...
export function ProductionStudio(props: ProductionStudioProps) {
  ...
  return <div className="production-rebuild">
    ...
    <header className="production-page-head"><div><span>ПРОИЗВОДСТВО</span><h2>Управление выпуском</h2><p>Каждый раздел открыт отдельно. Новый выпуск запускается только через пошаговый сценарий.</p></div><button className="button primary" onClick={openFlow}>Начать новый выпуск</button></header>
    <div className="production-workspace">
      <aside className="production-side-nav" aria-label="Разделы производства">
        <div className="production-status"><span>СЕЙЧАС</span><strong>{active.length ? `${active.length} партий в работе` : 'Производство свободно'}</strong><small>{ready.length ? `${ready.length} требуют решения` : `${inventory.length} складских лотов`}</small></div>
      ...
      <section className="production-section-content">
        {section === 'overview' && <ProductionOverview state={state} active={active.length} ready={ready.length} inventory={inventory.length} orders={pendingOrders.length} onStart={openFlow} onOpen={setSection} />}
...
function ProductionOverview({ state, active, ready, inventory, orders, onStart, onOpen }: { state: GameState; active: number; ready: number; inventory: number; orders: number; onStart: () => void; onOpen: (id: ProductionSection) => void }) { return <div className="production-overview"><header><span>ОБЗОР</span><h2>{active ? `${active} партий проходят цикл` : 'Следующий выпуск не запущен'}</h2><p>{ready ? `${ready} партий требуют решения.` : 'Линии готовы к новому производственному плану.'}</p><button className="button primary" onClick={onStart}>Начать новый выпуск</button></header><div className="production-overview-grid"><button onClick={() => onOpen('batches')}><span>Партии</span><strong>{active}</strong><small>в активном цикле</small></button><button onClick={() => onOpen('inventory')}><span>Склад</span><strong>{inventory}</strong><small>активных лотов</small></button><button onClick={() => onOpen('orders')}><span>Заказы</span><strong>{orders}</strong><small>ожидают поставки</small></button><button onClick={() => onOpen('facility')}><span>Объект</span><strong>{state.facility?.sanitation ?? 0}/100</strong><small>{state.facility ? 'чистота производства' : 'объект не выбран'}</small></button></div></div>; }
