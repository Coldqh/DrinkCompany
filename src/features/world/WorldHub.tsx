    ...
    <Modal title={organization.name} kicker={`${organizationKindLabel(organization.kind)} · ${statusLabel(organization.status)}`} onClose={onClose}>
      ...
      {(() => {
        ...
        return <>
          ...
          <WorldDisclosure title="Руководство" count={leaders.length}><div className="organization-assets">{leaders.map((leader) => <div key={leader.id}><strong>{leader.name}</strong><small>{leaderRoleLabel(leader.role)} · влияние {leader.influence} · риск {leader.riskTolerance} · лояльность {leader.loyalty}</small></div>)}</div></WorldDisclosure>
          {relations.length > 0 && <WorldDisclosure title="Связи" count={relations.length}><div className="organization-assets">{relations.map((relation) => { const otherId = relation.organizationAId === organization.id ? relation.organizationBId : relation.organizationAId; const other = ecosystem.organizations.find((item) => item.id === otherId); return <div key={relation.id}><strong>{other?.name ?? 'Неизвестная организация'}</strong><small>доверие {Math.round(relation.trust)} · зависимость {Math.round(relation.dependency)} · соперничество {Math.round(relation.rivalry)} · {relation.reason}</small></div>; })}</div></WorldDisclosure>}
          {memories.length > 0 && <WorldDisclosure title="Память компании" count={memories.length}><div className="organization-assets">{memories.map((memory) => <div key={memory.id}><strong>День {memory.day}</strong><small>{memory.summary}</small></div>)}</div></WorldDisclosure>}
        </>;
      })()}
      <WorldDisclosure title="Объекты" count={assets.length}><div className="organization-assets">{assets.length === 0 ? <small>Собственной недвижимости нет.</small> : assets.map((asset) => <div key={asset.id}><strong>{asset.name}</strong><small>{assetTypeLabel(asset.type)} · {asset.city} · {asset.status === 'for_sale' ? 'продаётся' : 'работает'}</small></div>)}</div></WorldDisclosure>
      {(() => {
        ...
        return <WorldDisclosure title="Первичный сектор" count={sites.length + processors.length}><div className="organization-assets">
...
function WorldDisclosure({ title, count, children, open = false }: { title: string; count?: number; children: ReactNode; open?: boolean }) {
  return <details className="world-disclosure" open={open}>
    <summary><span>{title}</span>{count !== undefined && <b>{count}</b>}<Icon name="arrow" /></summary>
    <div className="world-disclosure-body">{children}</div>
  </details>;
}
...
function ProductModal({ product, ecosystem, onClose }: { product: TradeProductState; ecosystem: NonNullable<GameState['ecosystem']>; onClose: () => void }) {
  ...
  return <Modal title={product.name} kicker={`${producer?.name ?? 'Производитель'} · ${productFamilyLabel(product.family)}`} onClose={onClose}>
    ...
    {(recentRuns.length > 0 || maturation.length > 0) && <WorldDisclosure title="Процессы и выдержка" count={recentRuns.length + maturation.length}><div className="organization-assets">{recentRuns.map((run) => <div key={run.id}><strong>{industrialStageLabel(run.stageId)} · {run.status === 'complete' ? 'завершён' : 'в работе'}</strong><small>{Math.round(run.inputVolumeLiters)} → {Math.round(run.outputVolumeLiters || run.inputVolumeLiters)} л · день {run.startDay}–{run.dueDay}</small></div>)}{maturation.map((lot) => <div key={lot.id}><strong>Выдержка · {lot.vesselType}</strong><small>{Math.round(lot.currentVolumeLiters)} л · {lot.ageDays} дн. · {lot.status}</small></div>)}</div></WorldDisclosure>}
    <WorldDisclosure title="Где продаётся" count={shelves.length}><div className="organization-assets">{shelves.length === 0 ? <small>Продукт ещё не попал на полки.</small> : shelves.map((listing) => { const asset = ecosystem.assets.find((item) => item.id === listing.assetId); return <div key={listing.id}><strong>{asset?.name ?? listing.assetId}</strong><small>{listing.units} осталось · сегодня {listing.unitsSoldToday} · цена {formatMoney(listing.retailPrice)}</small></div>; })}</div></WorldDisclosure>
    <WorldDisclosure title="Контракты и партии" count={contracts.length + batches.length}><div className="organization-assets">{contracts.map((contract) => { const buyer = ecosystem.organizations.find((item) => item.id === contract.buyerOrganizationId); return <div key={contract.id}><strong>{buyer?.name}</strong><small>{contract.quantity} ед. каждые {contract.intervalDays} дн. · {contract.lastResult}</small></div>; })}{batches.map((batch) => <div key={batch.id}><strong>{batch.status === 'ready' ? 'Готовая партия' : batch.status === 'blocked' ? 'Заблокирована' : 'В производстве'}</strong><small>{batch.producedUnits || batch.plannedUnits} ед. · {batch.issue ?? `день готовности ${batch.readyDay}`}</small></div>)}</div></WorldDisclosure>
  </Modal>;
}
