...
describe('startCompany', () => {
  it('создаёт рабочее состояние и списывает стоимость объекта', () => {
    ...
    expect(state.schemaVersion).toBe(25);
...
describe('save parsing', () => {
  it('принимает текущее сохранение schemaVersion 25', () => {
    const state = createCompany();
...
describe('save migration', () => {
  it('добавляет универсальную технологическую модель в сохранение schemaVersion 21', () => {
    ...
    const trade = ecosystem.trade as Record<string, unknown>;
    delete trade.industrial;
    const migrated = migrateGameState(legacy);
    expect(migrated.schemaVersion).toBe(25);
    expect(migrated.ecosystem?.trade.industrial).toBeDefined();
    expect(migrated.ecosystem?.trade.products.every((product) => Boolean(product.productionBlueprintId))).toBe(true);
  });

  it('добавляет финансовую экономику в сохранение schemaVersion 19', () => {
    const legacy = structuredClone(createCompany()) as unknown as Record<string, unknown>;
    legacy.schemaVersion = 19;
    const ecosystem = legacy.ecosystem as Record<string, unknown>;
    delete ecosystem.financials;
    const migrated = migrateGameState(legacy);
    expect(migrated.schemaVersion).toBe(25);
    expect(migrated.ecosystem?.financials.banks).toHaveLength(3);
    expect(migrated.ecosystem?.financials.accounts.length ?? 0).toBeGreaterThan(10);
    expect(migrated.ecosystem?.organizations.filter((organization) => organization.kind === 'financial')).toHaveLength(3);
  });
